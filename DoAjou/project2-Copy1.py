...
class Sentence2Vec:
    def __init__(self): 
        ...
        scores = {'메일' : 1, '이메일' : 1,'교수님' : 1 , '교수': 0.8,'학식':1,'기식':1,'오늘':0.9,'넘버':0.7,'소웨':0.8,\
         ...
         '사무실' : 1, '연구실' : 1, '랩실' : 1, '렙실' : 1, '어디':1,'학생식당':1,'기숙사식당':1,'학과사무실':1,'과사':0.8,'과사무실':1.0,'위치':0.8}
    ...
    def start(self,user_name,inputs):
        ...
        if professor_name == "1" :
            answer = "올바른 교수님의 성함을 입력해주세요. (소프트웨어과 교수님 한정)"
            files.file_overwrite_save(user_name,"0") # 0 = 교수님 성함 모름.
            return answer
        elif professor_name == "2" :
            answer = "교수님 한분의 성함만 입력해주세요. (소프트웨어과 교수님 한정)"
            files.file_overwrite_save(user_name,"0") # 0 = 교수님 성함 모름.
            return answer
        ...
        if similarity < 0.5 or len(inp) == 1 or intend == self.label_nt[12]:
